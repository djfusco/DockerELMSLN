...
#djf - leave this .sh pointing to orig /var/www/
...
if [[ $HOME != '/home/travis' ]]; then
  bash /usr/local/bin/elmsln-create-accounts.sh
  # apply ssh settings for innovate contributions
  bash /usr/local/bin/apply-ssh.sh
...
elmslnecho "bash /usr/local/bin/elmsln-install.sh"
